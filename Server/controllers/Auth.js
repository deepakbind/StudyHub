...
exports.login = async (req, res) => {
  try {
    ...
    const token = jwt.sign({ id: user._id, email: user.email, role: user.accountType }, process.env.JWT_SECRET, { expiresIn: "24h" });
...
exports.changePassword = async (req, res) => {
  try {
    ...
    if (!isPasswordMatch) return res.status(401).json({ success: false, message: "The old password is incorrect" });

    const encryptedPassword = await bcrypt.hash(newPassword, 10);
    const updatedUser = await User.findByIdAndUpdate(req.user.id, { password: encryptedPassword }, { new: true });

    try {
      await mailSender(updatedUser.email, "Password Updated", passwordUpdated(updatedUser.email, `Password updated successfully for ${updatedUser.firstName} ${updatedUser.lastName}`));
    } catch (err) {
